...
from backend.app.schemas import (
    TopicCurateResponse,
    TopicListResponse,
    TopicMasteryUpdateRequest,
    TopicPracticeQuestionsResponse,
    TopicRefreshResponse,
    TopicResponse,
    TopicReviewResponse,
    TopicUpdateRequest,
)
from backend.app.services.topics import (
    curate_course_topics,
    generate_topic_practice_questions,
    get_course_topic,
    get_topic_review_payload,
    list_course_topics,
    refresh_course_topics,
    serialize_topic,
    update_topic,
)
...
@router.post("/{topic_id}/practice-questions", response_model=TopicPracticeQuestionsResponse)
async def create_topic_practice_questions(
    course: Course = Depends(get_user_course_or_404),
    topic: Topic = Depends(get_user_topic_or_404),
    db: Session = Depends(get_db),
):
    if topic.course_id != course.id:
        raise HTTPException(status_code=404, detail="Topic not found")
    questions = generate_topic_practice_questions(db, topic=topic)
    return TopicPracticeQuestionsResponse(topic_id=topic.id, questions=questions)
