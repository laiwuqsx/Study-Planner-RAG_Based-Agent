...
def main() -> None:
    ...
    auth = request_json("/auth/register", method="POST", payload={"username": f"phase6_{suffix}", "password": "pass123"})
    token = auth["access_token"]
    course = request_json(
        "/courses",
        method="POST",
        token=token,
        payload={"name": f"Chat {suffix}", "term": "Spring 2026", "description": "Phase 6 smoke"},
    )
    with tempfile.TemporaryDirectory() as temp_dir:
        docx_path = pathlib.Path(temp_dir) / "chat.docx"
        ...
        chat = request_json(
            ...
            method="POST",
            token=token,
            payload={"message": "How do threads differ from processes?", "retrieval_mode": "keyword", "top_k": 4},
        )
        sessions = request_json(f"/courses/{course['id']}/sessions", token=token)
        detail = request_json(f"/sessions/{chat['session']['id']}", token=token)
        ...
        json.dumps(
            {
                ...
                "assistant_sources": len(chat["assistant_message"]["sources"]),
                "assistant_preview": chat["assistant_message"]["content"][:120],
            },
            ensure_ascii=False,
        )
    )
